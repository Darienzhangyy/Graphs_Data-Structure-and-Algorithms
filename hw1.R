...
#
# Description - Constructs a valid graph object with (length) vertices and edge weights no larger than (maxWeight).
...
randomGraph = function(length, maxWeight=10, seed=NULL) {
  ...
  filler = function(x) {
    n = round(runif(1, 1, sqrt(length)))
    uniqueEdges = unique(as.integer(round(runif(n, 1, length))))
    n = length(uniqueEdges)
    filled = list(edges=uniqueEdges, 
                  weights=as.numeric(round(runif(n, 1e-10, maxWeight))))
    return(filled)
  }
  g = lapply(g, filler)
...
is_connected = function(g, v1, v2, unweighted=F) {
...
#          V1, a vertex label in G; 
#          V2, a vertex label in G; 
#          Warn, the content of any warning; Exit, whether to return from function;
#          Unnamed, whether to convert labels to indexes.
#
# Description - Labels graph and ensures vertices are valid matches.

tidyLabels = function(g, v1, v2) {
  warn = NULL
  exit = F
  unnamed = ifelse(is.null(names(g)), T, F)
  ...
  if(!all(c(v1, v2) %in% names(g))) {
    ...
    exit = T
    return(list(G=g, V1=v1, V2=v2, Warn=warn, Exit=exit, Unnamed=unnamed))
  }
  return(list(G=g, V1=v1, V2=v2, Warn=warn, Exit=exit, Unnamed=unnamed))
}
...
is_undirected = function(g) {
  ...
  a = adjacencyMatrix(g)
  out = ifelse(identical(a, t(a)), T, F)
  return(out)
}
...
#         v1, a vertex label in g; 
#         v2, a vertex label in g;
#         weight, a logical indicating whether the edge weight of the shortest path should be reported
#                 (defaults to FALSE).
#
...
#          as the first element and the edge weight ($pathWeight) as the second element.
#
...
shortest_path = function(g, v1, v2, weight=F) {
  if(!any(is.character(c(v1, v2)) | mode(c(v1, v2))=='numeric')) { 
  ...
  if(!suppressWarnings(is_connected(g, v1, v2))) { 
    return(warning('Unconnected vertices; please enter connected vertices.', call.=F)) 
  } 
  a = adjacencyMatrix(g)
  best = Inf
  ...
  while(!(is.null(pathDn) | is.null(pathUp))) {
    ...
    if(!is_undirected(g)) {
      pathDn = ifelse(length(which(tails %in% oldTails))>0, pathDn[-which(tails %in% oldTails)], pathDn)
    }
    ...
    tails = getTails(pathDn)
    if(!is_undirected(g)) {
      pathUp = ifelse(length(which(heads %in% oldHeads))>0, pathUp[-which(heads %in% oldHeads)], pathUp)
    }
    linkCheck = is_linked(pathDn, pathUp, heads, tails)
    if(linkCheck$linked==T) { 
      possBest = linkCheck$best
...
Testing = function(x) { cat('Testing') }
# Testing
...
is_connected(graph3, 1, 6)     # T
is_connected(graph3, '1', '6') # T + warning
is_connected(graph3, 'A', 'F') # T + warning
is_connected(graph4, 3, 5)     # T
...
is_connected(graph3, 35)       # warning
is_connected(graph3, 1, 2, 3)  # warning
is_connected(graph8, 1, 2)     # T
...
connectionChecker(graph1) # system.time() indicates no significant difference between is.connected() and is_connected() for
connectionChecker(graph2) # small graphs.
connectionChecker(graph3)
connectionChecker(graph4)
connectionChecker(graph5)
connectionChecker(graph6)
connectionChecker(graph7)
connectionChecker(graph8)
connectionChecker(graph9)

is_undirected(graph1) # F
is_undirected(graph2) # F
is_undirected(graph3) # F
is_undirected(graph4) # F
is_undirected(graph5) # F
is_undirected(graph6) # F
is_undirected(graph7) # F
is_undirected(graph8) # T
is_undirected(graph9) # F
...
realDeal = c(F,T,F,F,T,F,F,F,F,T,F,F,F,F,F,F,T,T,F,F,F,F,F,F,F,F,T,F,F,F,F,F,F,F,F,F,rep(T, 9))
toVerify = c(apply(apply(combn(seq(9), 2), 
                         1, function(x) { paste0('graph', x) } ), 
                   1, function(y) {is_isomorphic(get(y[1]), get(y[2]))} ),
             apply(apply(matrix(1, nrow=2, ncol=9), 
                         1, function(x) { paste0('graph', x) } ), 
                   1, function(y) {is_isomorphic(get(y[1]), get(y[2]))} ))
identical(realDeal, toVerify)
rm(list=c('realDeal', 'toVerify'))

is.connected(graph1, 'A', 'F') # T
is.connected(graph2, 'A', 'F') # T
is.connected(graph1, 1, 6)     # T
is.connected(graph1, 'F', 'a') # F
is.connected(graph1, 'e', 'a') # F
is.connected(graph2, 'D', 'N') # T
is.connected(graph3, 1, 6)     # T
is.connected(graph3, '1', '6') # T + warning
is.connected(graph3, 'A', 'F') # T + warning
is.connected(graph4, 3, 5)     # T
is.connected(graph2, 'b', 'a') # T
is.connected(graph7, 'x', 'z') # F + warning
is.connected(graph1, T, F)     # F + warning
is.connected(graph3, 35, 83)   # F + warning
is.connected(graph3, 35)       # warning
is.connected(graph3, 1, 2, 3)  # warning
is.connected(graph8, 1, 2)     # T
is.connected(graph8, 3, 4)     # T
is.connected(graph8, 3, 5)     # T
is.connected(graph8, 'A', 'B') # T
is.connected(graph8, 'D', 'n') # T
is.connected(graph5, 'A', 'B') # F

shortest_path(graph1, 'A', 'F') # A-B-C-E-F
shortest_path(graph2, 'D', 'N') # D-A-B-F-N
shortest_path(graph1, 'F', 'a') # warning
shortest_path(graph3, 1, 6)     # 1-2-3-5-6
shortest_path(graph3, 'A', 'F') # A-B-C-E-F warning
shortest_path(graph4, 3, 5)     # 3-1-2-4-5
...
shortest_path(graph8, 3, 4)     # D-A-B-F
shortest_path(graph8, 3, 5)     # D-A-N
shortest_path(graph5, 'A', 'B') # warning
shortest_path(graph8, 3, 2)     # D-A-B

graphOut = randomGraph(25)
gSeed = graphOut$seed
g = graphOut$graph

adjacencyMatrix(g)
is_connected(g, 1, 25)
is_connected(g, 1, 25, unweighted=T)
connectionChecker(g)
is_undirected(g)
is_isomorphic(g, test)
shortest_path(g, 1, 25, T)


t1 = randomGraph(1000)$graph
t2 = randomGraph(1000)$graph
is_isomorphic(t1, t1)
is_isomorphic(t1, t2)
system.time(is_connected(t1, 1, 100))
connectionChecker(t1)