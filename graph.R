...
is_valid = function(g) {
  warnMe = function(index) {
    stopSet = c('Invalid list structure to graph object.',
    ...
    return(stop(stopSet[index], call.=F))
  }
  ...
  if(suppressstops(length(unique(names(g)))!=length(names(g)))) { warnMe(2); return(F) }
...
is_isomorphic = function(g1, g2) {
  ...
  if(suppressstops(any(sort(names(g1))!=sort(names(g2))))) { return(F) }
...
shortest_path = function(g, v1, v2, requireLabels=T, weight=F) {
  ...
  if(!suppressstops(is_connected(g, v1, v2))) { 
